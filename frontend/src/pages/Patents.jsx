...
import { Search, Gavel, FileCheck, Calendar, ArrowRight, ShieldCheck, HelpCircle, RefreshCw } from 'lucide-react';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedPatent ? '1.8fr 1.2fr' : '1fr', gap: '1.5rem', transition: 'all 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {query.trim().length >= 3 && !isValidSearchQuery(query) && (
            <div style={{ color: 'var(--color-error)', fontSize: '0.75rem', fontFamily: 'var(--font-mono)', marginTop: '-0.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {loading && patents.length === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.1)', border: '1px solid var(--border-color)', borderRadius: '6px' }}>
                <RefreshCw size={16} className="glow-text-pink" style={{ animation: 'spin 2s linear infinite', marginRight: '0.5rem' }} />
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>AUDITING PATENT REGISTRIES...</span>
              </div>
            ) : patents.length === 0 ? (
              <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.1)', border: '1px solid var(--border-color)', borderRadius: '6px', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', maxWidth: '75%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{pat.classifications}</span>
                    </div>
                    <h3 style={{ fontSize: '1.05rem', fontWeight: 700 }}>{pat.title}</h3>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      Patent ID: <strong style={{ color: 'var(--color-primary)', fontFamily: 'var(--font-mono)' }}>{pat.id}</strong> | Inventor: <strong>{pat.inventor}</strong>
                    </p>
                  </div>
          ...
          <div className="glass-panel animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', background: 'rgba(10,13,20,0.9)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem' }}>
              <div>
                ...
                <span style={{ fontSize: '0.75rem', fontFamily: 'var(--font-mono)', color: 'var(--color-secondary)' }}>{selectedPatent.id}</span>
              </div>
              <button 
            ...
            <div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', fontFamily: 'var(--font-mono)' }}>ABSTRACT</span>
            ...
            <div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', fontFamily: 'var(--font-mono)', marginBottom: '0.35rem' }}>INDEPENDENT CLAIMS</span>
