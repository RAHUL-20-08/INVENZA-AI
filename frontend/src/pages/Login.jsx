...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleCredentialsSubmit = async (e) => {
    ...
    setIsLoggingIn(true);
    setLogs([
      `[AUTH] Initiating secure credential handshake...`,
      `[AUTH] Querying authorization node for: ${emailInput}`
    ]);
    ...
    try {
      ...
      if (res.ok && data.success) {
        setLogs(prev => [
          ...prev,
          `[AUTH] Credentials verified. Signature matches database record.`,
          `[ROLE] Assigned security clearance: ${data.user.role.toUpperCase()}`,
          `[SYS] Establishing secure session token...`
        ]);

        setTimeout(() => {
          setLogs(prev => [
            ...prev,
            "[AUTH] Security clearance verification: GRANTED",
            "[SYS] Launching secure analyst database terminal..."
          ]);
        }, 600);
        ...
        setIsLoggingIn(false);
        setLogs([]);
        setErrorMessage(data.message || 'Verification failed. Incorrect email or password.');
      }
    } catch (err) {
      setIsLoggingIn(false);
      setLogs([]);
      setErrorMessage('Network error connecting to authorization server.');
    }
  };
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    setIsLoggingIn(true);
    setLogs([
      `[SYS] Connecting to registration ledger...`,
      `[SYS] Processing new credentials for email: ${emailInput}`
    ]);
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setErrorMessage(data.message || 'Registration failed. Email may already exist.');
      }
    } catch (err) {
      setIsLoggingIn(false);
      setLogs([]);
...
=======
        ...
        <p style={{
          ...
          Clearance credentials must be verified through official federated single sign-on (SSO) nodes.
        </p>
        ...
        {/* Credentials Form or Terminal Logs */}
        {isLoggingIn ? (
...
=======
          ...
          <form onSubmit={isRegistering ? handleRegisterSubmit : handleCredentialsSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <label style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                {isRegistering ? 'Email address' : 'Username / Email'}
              </label>
              <input
                type={isRegistering ? 'email' : 'text'}
                required
                className="tech-input"
                placeholder={isRegistering ? 'Enter your email' : 'Enter your username or email'}
                value={emailInput}
        ...
        {/* Divider (Only display if not loading) */}
        {!isLoggingIn && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', margin: '0.5rem 0' }}>
            <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
            <span style={{ fontSize: '0.65rem', color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-mono)' }}>
              Or Continue With
            </span>
            <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
          </div>
        ...
        {/* Federated Login Buttons (Only display if not loading) */}
        {!isLoggingIn && (
